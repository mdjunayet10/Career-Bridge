...
- `SMTP_HOST`, `SMTP_PORT`, `SMTP_SECURE`, `SMTP_USER`, `SMTP_PASS`, `EMAIL_FROM`: email settings used to send password reset verification codes
...
Password reset uses hashed one-time verification codes. With SMTP configured, Career Bridge emails a 6-digit code to the account email. The code is not shown in the browser. For isolated local demos only, `SHOW_DEVELOPMENT_RESET_CODE=true` can expose the code outside production.
